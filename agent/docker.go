...
func (dm *dockerManager) containerAction(containerID string, action string) error {
	if action != "start" && action != "stop" && action != "restart" {
		return fmt.Errorf("invalid action: %s", action)
	}
	apiURL := fmt.Sprintf("http://localhost/containers/%s/%s", containerID, action)
	resp, err := dm.client.Post(apiURL, "application/json", nil)
	if err != nil {
		return err
	}
	defer resp.Body.Close()
	if resp.StatusCode >= 400 {
		...
		return fmt.Errorf("docker API returned error %d: %s", resp.StatusCode, string(bodyBytes))
	}
	return nil
}
